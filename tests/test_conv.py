...
class Convolution2DFunctionTest(unittest.TestCase):
    ...
    def test_Convolution2DFunctionImpl1_fprop(self):
        ...
        filters = torch.randn([K,C//G,R,S], dtype=torch.float64, requires_grad=True)
        bias = torch.randn([K], dtype=torch.float64, requires_grad=True)
   
        ref_y = torch.nn.functional.conv2d(x, filters, bias, (U,V), (PH,PW), (DH,DW), G)
        tst_y = conv(x, filters, bias, (U,V), (PH,PW), (DH,DW), G)
    ...
    def test_Convolution2DFunctionImpl1_bprop(self):
        ...
        self.assertTrue(torch.autograd.gradcheck(conv, (x, filters, bias, (U,V), (PH,PW), (DH,DW), G)))
